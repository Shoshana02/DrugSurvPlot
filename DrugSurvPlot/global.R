...
# 2.1 load data--------------------
 load('www/data/druglist.Rdata')
 load('www/data/final_group.Rdata') # grouplist
 drug <- as.character(druglist$drug) # drug
 load("www/data/download_dt_test.Rdata") #info_agg_dt

# 2.2 read_select_dt-----------------------------------------
read_select_dt <- function (x) {
  # load(file = 'www/data/final_group.Rdata')
  file_path <- paste0('www/data/GeneDatatable/drug_',final_group$group[which(final_group$drug==x)],'.Rdata')
  load(file = file_path)
  subdt <- subdt[which(subdt$Drug == x),]
  #subdt <- subdt[,c(1:6,10:12,7:9,13:16,20:22,17:19)]
  subdt
}

 # read_select_dt <- function (x) {
 #   subdt <- GEO_ALL_demo[which(GEO_ALL_demo$Drug == x),]
 #   #subdt <- subdt[,c(1:6,10:12,7:9,13:16,20:22,17:19)]
 #   subdt
 # }
# 2.3 create_btns-------------------------------------------------------------
...
drug_km_plot <- function(cancertype, gse, gpl, survival, drug,  cutoff, col_1, col_2) {
  tryCatch(
    { 
      exp_file <- paste0("www/data/", cancertype, "/", gse, "/", survival, "_drugsens", cutoff, "_", gpl, ".Rdata")
      ...
      cli<-na.omit(cli)
      exp <- as.data.frame(drugsens[,drug])
      colnames(exp) <- drug
      rownames(exp) <- rownames(drugsens)
      ...
      final_drug<-data.frame(
        row.names = paste0(paste0('A',c(1:c(ncol(rt)-2)))),
        drug = colnames(rt)[3:ncol(rt)],
        drug1 = paste0(paste0('A',c(1:c(ncol(rt)-2)))))
      ...
      if(nchar(drug)>52){
       drug_re <- paste0(substr(drug,1,52),"\n",substring(drug,53))
      }else{
        drug_re <- drug
      }
      ...
      ggsurvplot(fit,
                 ...
                 surv.median.line='hv',
                 legend=c(0.8,0.8),
                 legend.title = drug_re,
                 legend.labs = c(paste0("Low Predicted Sensitivity Scores"," (",fit$n[1],")"),
                                 paste0("High Predicted Sensitivity Scores"," (",fit$n[2],")")),
