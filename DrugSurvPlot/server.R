...
function(input, output, session) {
  ...
  updateSelectizeInput(session, inputId = "select.drug", 
                       choices = drug, 
  ...
  observeEvent(input$select.drug,{
    output$syn <- renderText({druglist$syn[which(druglist$drug==input$select.drug)]
    })#show brief description
    output$tar <- renderText({druglist$tar[which(druglist$drug==input$select.drug)]
    })#show brief description
    output$tar_path <- renderText({druglist$tar_path[which(druglist$drug==input$select.drug)]
    })#show brief description
    output$web <- renderUI({
      tagList("URL link:", a("Jump to the GDSC2 Website",href=druglist$website[which(druglist$drug==input$select.drug)]))
    })#web link
  })
  ...
  shinyjs::disable(id = "drug_tablebox")
  shinyjs::disable(id = "drug_plotbox")
  ...
  dataset <- eventReactive(input$submitdata_btn, {
    final_dt_test <- read_select_dt(input$select.drug) %>% 
  ...
  output$downloaddata_btn <- downloadHandler(
    filename = function(){
      paste(input$select.drug,"_ResultsDatatable.csv",sep="")
    },
  ...
  observeEvent(input$submitdata_btn,{
    if (input$drug_tablebox$collapsed) {
      updateBox("drug_tablebox", action = "toggle")
    }
    shinyjs::enable(id = "drug_tablebox")
    ...
    output$drugresultdt <- shiny::renderUI({
      ...
      DT::datatable(
        ...
        options = list(
          ...
          columnDefs = list(list(className = 'dt-center', targets = "_all"),
                            ...
                            list(width = '70px', targets = c(21)),
                            list(searchable = FALSE, targets = c(7:21)), # disable the filter function in columns "HR/CI/P"
  ...
  shiny::observeEvent(input$current_id, {
    ...
    if (!is.null(input$current_id) & stringr::str_detect(input$current_id, pattern = "choosem")) {
      output$DTselectinfo <- renderText(paste("You are interested in <b>", symbol_show(),  "</b>.", 
                                              ...
                                              "<br>",
                                              "<br>", "<b> You can switch to the OPTIMAL cut-off method by clicking the GREY button alongside. More colors can be customized below. </b>"))
      drug_plot <- drug_km_plot(cancertype = dt_id_split[[1]][2],
                                gse = dt_id_split[[1]][3],
                                gpl = dt_id_split[[1]][4],
                                survival = dt_id_split[[1]][5],
                                drug = dt_id_split[[1]][1],
                                #gene_show = symbol_show(),
                                cutoff = "m",
                                col_1 = "#c35f50",
                                col_2 = "#477aae")
      output$KMdrug_plot <- renderPlot({drug_plot})
      output$download_KMdrug<- downloadHandler(
        filename = function(){
          paste("KM_drug_",dt_id_split_re()[[1]][1],".pdf",sep="")
        },
        content = function(file) {
          cairo_pdf(file)
          print(drug_plot)
    ...
    if (input$drug_plotbox$collapsed) {
      updateBox("drug_plotbox", action = "toggle")
    }
    shinyjs::enable(id = "drug_plotbox")
    scroll("drug_plotbox", block = "end")
    # runjs('
    #   window.scrollTo(0, document.body.scrollHeight);
    # ')
  })
  
  observeEvent(input$customize_drugbtn, {
    dt_id_split <- dt_id_split_re()
    drug_plot <- drug_km_plot(cancertype = dt_id_split[[1]][2],
                                    gse = dt_id_split[[1]][3],
                                    gpl = dt_id_split[[1]][4],
                                    ...
                                    drug = dt_id_split[[1]][1],
                                    #gene_show = symbol_show(),
                                    cutoff = input$cutoff_KMdrug,
                                    col_1 = input$drug_low_color,
                                    col_2 = input$drug_high_color)
    output$KMdrug_plot <- renderPlot({drug_plot})
    output$download_KMdrug<- downloadHandler(
      filename = function(){
        paste("KM_drug_",dt_id_split_re()[[1]][1],".pdf",sep="")
  ...
  observeEvent(input$customize_defalutbtn, {
    updateColorPickr(session, inputId = "drug_low_color", value = "#c35f50")
    updateColorPickr(session, inputId = "drug_high_color", value = "#477aae")
    updateRadioButtons(session, inputId = "cutoff_KMdrug", selected = "m")
  }) 
  # 3 box_KMinfodt_box  -----------------------------------------------------
  ...
  observeEvent(input$submit_commentbtn, {
    ...
    email <- envelope() %>%
      ...
      cc("shenqr@i.smu.edu.cn") %>%
      subject(paste("DrugSurvPlot-SHINY_FEEDBACK: ", input$contact))%>%
